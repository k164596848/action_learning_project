...
def save_to_json_path(video_path, folder_path, **kwargs) -> bool:#openpose轉成json 格式
...
def run(video_path: str, folder_path: str, hand=False) -> bool:#main program runing
    #for only one person   
    # command = f"{get_executor_path()} --video '{abspath(video_path)}' --number_people_max 1 --tracking 0 --write_json {abspath(folder_path)} --display 0 --render_pose 0"
    #for two more people 
...
def run_test(video_path: str, folder_path: str, hand=False) -> bool:#main program runing  
    command = f"{get_executor_path()} --video '{abspath(video_path)}' --num_gpu_start 0 --number_people_max -1 --write_json {abspath(folder_path)} --display 0 --render_pose 0"
    # command = f"{get_executor_path()} --video '{abspath(video_path)}' --number_people_max 1 --tracking 0 --write_json {abspath(folder_path)} --display 0 --render_pose 0"
    if hand:
        command += " --hand"
    is_success = exec_command(command)
